...
async function getVideoList(page = 1, keyword = '') {
  try {
    ...
    if (keyword) {
      ...
      items = (data?.data?.result || []).filter((item) => item.type === "video");
    } else {
      // 热门推荐模式
      logInfo(`获取热门推荐, 页码: ${page}`);
    ...
    const promises = items.map(async (item) => {
      ...
      return {
        ...
        vod_pic: cover || '',
        vod_remarks: item.duration ? formatDuration(item.duration) : (item.duration || ''),
      };
    });
...
async function category(params) {
  const categoryId = params.categoryId || "recommend";
  const pg = parseInt(params.page) || 1;
  
  try {
    logInfo(`获取分类数据: ${categoryId}, 页码: ${pg}`);
    
    // 获取指定页的数据
    const list = await getVideoList(pg);
    
    logInfo(`分类返回 ${list.length} 个视频`);
    
    return {
      list: list,
      page: pg,
      pagecount: 100, // 假设有100页
      total: list.length * 100 // 估算总数
    };
  } catch (error) {
...
async function search(params) {
  ...
  try {
    ...
    const list = await getVideoList(pg, wd);
