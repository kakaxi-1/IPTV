...
const logInfo = (message, data = null) => {
    ...
    OmniBox.log("info", `[hsbox-DEBUG] ${output}`);
};

const logError = (message, error) => {
    OmniBox.log("error", `[hsbox-DEBUG] ${message}: ${error.message || error}`);
};
...
function formatVideos(list) {
    ...
    return list.map(item => {
        ...
        return {
            ...
            vod_play_from: String(item.vod_play_from || 'hsckyun'),
...
const parsePlaySources = (vodItem) => {
    ...
    const playFrom = vodItem.vod_play_from || 'hsckyun';
    
    const playFroms = playFrom.split(',');
    
    for (let i = 0; i < playFroms.length; i++) {
        const sourceName = playFroms[i] || `线路${i+1}`;
        const playPageUrl = `${API_HOST}/play/${vodId}.html?from=${i}`;
        
        const episodes = [{
            name: '正片',
            playId: `${playPageUrl}|||${encodeMeta({ sid: vodId, fid: `${vodId}#${i}`, v: vodName, e: 1 })}`,
            _fid: `${vodId}#${i}`,
            _rawName: '正片'
        }];
